...
### AlphaFold Run — Dome-24 (ACTIVE — started June 17, 2026 ~05:57 CDT)
- **Script**: `job_dome24_bigmem.sh` — SLURM job **50737753** (resubmission; original 50698644 was replaced)
- **Partition**: bigmem (1 node, 48 CPUs, 768 GB RAM, no GPU), 36h wall time; running on midway3-0318
...
- **Template**: `--max_template_date=2026-06-15` — includes 9cz2 as structural template; known regions are templated, gap regions predicted with full dome context
- **Goal**: Fill in all three missing regions simultaneously: HflK M3 (356–419) on all 12 chains, HflC 161–190 on all 12 chains, lower halves of chains X/V/W
- **Output**: 5 predicted structures at `af2_dome24_output/dome_24chain_input/`; expected first model ~15–20h from start → evening/night of June 17
- **Inference**: CPU-only (JAX_PLATFORM_NAME=cpu, XLA 48-core parallel)
- **Post-run plan**: Download `ranked_0.pdb`, trim HflK resid 1–78, verify M3 inward orientation, check clashes, submit to CHARMM-GUI Membrane Bilayer Builder for dome-only system
...
- **Decision**: Abandoned. Waiting for dome-24 result which predicts all 24 M3 tails simultaneously in dome context → correct inward orientation guaranteed by multimer modeling
...
│   ├── job_dome24_bigmem.sh          # 24-chain dome run (bigmem, job 50737753 RUNNING) ← ACTIVE
...
│   ├── af2_dome24_output/            # Dome-24 output (in progress; ranked_0.pdb not yet available)
