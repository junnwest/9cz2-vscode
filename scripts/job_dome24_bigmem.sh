#!/bin/bash
#SBATCH --job-name=af2_dome24_bigmem
...
#SBATCH --qos=bigmem-prio
#SBATCH --output=af2_dome24_bigmem_%j.out
#SBATCH --error=af2_dome24_bigmem_%j.err
...
echo "Chain count in FASTA: $(grep -c '^>' $FASTA)"
echo "Precomputed MSA dirs: $(ls $OUTPUT_DIR/dome_24chain_input/msas/)"

# Launch AF2 in background so we can monitor it
...
# Background monitor: logs every 10 min to a separate file
(
  MONITOR_START=$SECONDS
  echo "$(date '+%Y-%m-%d %H:%M:%S') | monitor start | AF2 PID: $AF2_PID" > "$MONITOR_LOG"
  echo "timestamp | elapsed | cpu% | rss_gb | models_done/5 | unrelaxed_pdbs" >> "$MONITOR_LOG"
  while kill -0 "$AF2_PID" 2>/dev/null; do
    ...
    PKLS=$(find "$MODEL_OUT" -name 'result_model_*.pkl' 2>/dev/null | wc -l)
    PDBS=$(find "$MODEL_OUT" -name 'unrelaxed_*.pdb' 2>/dev/null | wc -l)
    echo "$(date '+%Y-%m-%d %H:%M:%S') | ${H}h${M}m | ${CPU}% | ${RSS_GB}GB | ${PKLS}/5 | ${PDBS}" >> "$MONITOR_LOG"
  done
  echo "$(date '+%Y-%m-%d %H:%M:%S') | AF2 PID $AF2_PID exited" >> "$MONITOR_LOG"
) &
MONITOR_PID=$!

# Wait for AF2; propagate its exit code
